...
const FONT_FAMILY =
  'Sakr, -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, sans-serif'
      ...
      case 'textarea':
        ...
        if (typeof v !== 'string') {
          errs.push({
            field: f.name,
            code: 'INVALID',
            message: t('Invalid text', 'نص غير صالح'),
          })
          break
        }
        if (
          (f as any).props?.max_length &&
          v.length > (f as any).props.max_length
        ) {
          errs.push({
            field: f.name,
            code: 'TOO_LONG',
            message: t('Too long', 'طويل جداً'),
          })
        }
        if ((f as any).props?.pattern) {
          try {
            const re = new RegExp((f as any).props.pattern)
            if (!re.test(v))
              errs.push({
                field: f.name,
                code: 'PATTERN',
                message: t('Does not match pattern', 'لا يطابق النمط'),
              })
          } catch {}
        }
        break
      case 'number':
        ...
        if (Number.isNaN(num)) {
          errs.push({
            field: f.name,
            code: 'INVALID',
            message: t('Invalid number', 'رقم غير صالح'),
          })
          break
        }
        if ((f as any).props?.min != null && num < (f as any).props.min)
          errs.push({
            field: f.name,
            code: 'MIN',
            message: t('Too small', 'صغير جداً'),
          })
        if ((f as any).props?.max != null && num > (f as any).props.max)
          errs.push({
            field: f.name,
            code: 'MAX',
            message: t('Too large', 'كبير جداً'),
          })
        break
      case 'select':
        if (v == null) break
        ...
        if (!(f as any).props?.allow_custom && !(f as any).props?.allow_other) {
          ...
          if (!found && v?.value !== 'other') {
            errs.push({
              field: f.name,
              code: 'NOT_ALLOWED',
              message: t('Not in options', 'غير موجود ضمن الخيارات'),
            })
          }
        }
      ...
      case 'multiselect':
        ...
        if (!(f as any).props?.allow_custom && !(f as any).props?.allow_other) {
          ...
          for (const it of Array.isArray(v) ? v : []) {
            if (
              !options.some((o) => o.value === it?.value) &&
              it?.value !== 'other'
            ) {
              errs.push({
                field: f.name,
      ...
      case 'email':
        ...
        if (!/^[A-Za-z0-9._%+\-]+@[A-Za-z0-9.\-]+\.[A-Za-z]{2,}$/.test(em))
          errs.push({
            field: f.name,
            code: 'INVALID_EMAIL',
            message: t('Invalid email', 'بريد إلكتروني غير صالح'),
          })
        break
      case 'phone':
        if (v == null) break
        ...
        if ((f as any).props?.e164_required) {
          if (!/^\+[1-9]\d{7,14}$/.test(phoneObj?.e164 || '')) {
            errs.push({
              field: f.name,
              code: 'E164',
              message: t('Invalid E.164', 'تنسيق E.164 غير صالح'),
            })
          }
        }
        ...
        if (!phoneObj?.e164) {
          errs.push({
            field: f.name,
            code: 'REQUIRED',
            message: t('Phone number required', 'رقم الهاتف مطلوب'),
          })
        }
        break
      case 'file_upload':
        ...
        if (arr.length > maxFiles)
          errs.push({
            field: f.name,
            code: 'TOO_MANY',
            message: t('Too many files', 'عدد ملفات كبير'),
          })
        break
      case 'location':
        if (v == null) break
        if (typeof v !== 'object' || v === null || Array.isArray(v)) {
          errs.push({
            field: f.name,
            code: 'INVALID',
            message: t('Invalid location', 'موقع غير صالح'),
          })
          break
        }
        ...
        if (hasLat && hasLng) {
          ...
          if (lat < -90 || lat > 90)
            errs.push({
              field: f.name,
              code: 'INVALID',
              message: t('Invalid latitude', 'خط عرض غير صالح'),
            })
          if (lng < -180 || lng > 180)
            errs.push({
              field: f.name,
...
const ListingGalleryCard: React.FC<{
  ...
  const description =
    descParts.join(' • ') || (locale === 'ar' ? 'إعلان' : 'Ad')

  return (
    <div
      style={
        isSelected
          ? galleryCardSelectedStyle
          : isHovered
            ? galleryCardHoverStyle
            : galleryCardStyle
      }
      onClick={onSelect}
    ...
    >
      {isSelected && <div style={galleryCheckmarkStyle}>✓</div>}
      {listing.thumbnail ? (
        <img
...
export const FormView: React.FC<{
  ...
  const effectiveLocale =
    (locale as any) || (getLocaleFromURL(form.default_locale) as any)
  ...
  const [purchaseAuthConfig, setPurchaseAuthConfig] =
    React.useState<PurchaseAuthConfig | null>(null)
  ...
  const [userId, setUserId] = React.useState<string | number | undefined>(
    undefined,
  )
  const [formStartTime] = React.useState<number>(Date.now())
  const [fieldFocusTimes, setFieldFocusTimes] = React.useState<
    Record<string, number>
  >({})
  const [lastFieldInteracted, setLastFieldInteracted] = React.useState<
    { name: string; type: string } | undefined
  >(undefined)
  const [submissionId, setSubmissionId] = React.useState<number | undefined>(
    undefined,
  )
  ...
  React.useEffect(() => {
    const checkAuthOnLoad = async () => {
      ...
      if (storedToken) {
        ...
        if (validation.valid) {
          console.log(
            '[FormView] Token is valid, fetching listings and showing form',
          )
          setAuthToken(storedToken)

          // Fetch user listings for valid stored token
          const listingsResult = await fetchMyListings(
            storedToken,
            authCfg,
            effectiveLocale,
          )
          if (listingsResult.success) {
            ...
            if (listingsResult.user) {
              console.log(
                '[FormView] Setting userData from stored token listings:',
                JSON.stringify(listingsResult.user),
              )
              setUserData({
  ...
  const handleLogin = async (
    ...
    if (result.success && result.accessToken) {
      ...
      if (listingsResult.success) {
        ...
        if (listingsResult.user) {
          console.log(
            '[FormView] Setting userData from listings response:',
            JSON.stringify(listingsResult.user),
          )
          setUserData({
            id: listingsResult.user.id,
  ...
  React.useEffect(() => {
    ...
    try {
      initAmplitude(extractedSessionId)
        ...
        .catch((error) => {
          console.warn(
            '[FormView] Amplitude initialization failed, continuing without analytics:',
            error,
          )
        })
    } catch (error) {
      ...
      console.warn(
        '[FormView] Amplitude setup error, continuing without analytics:',
        error,
      )
    }
  ...
  React.useEffect(() => {
    const fetchItemPrice = async () => {
      ...
      if (!variantId) {
        console.error(
          '[FormView] Unknown item value, cannot map to variant:',
          itemValue,
        )
        setSelectedItemPrice(null)
      ...
      try {
        const baseUrl =
          purchaseAuthConfig.auth_api_base_url ||
    ...
    purchaseAuthConfig,
    authToken,
    answers[purchaseAuthConfig?.item_id_field || ''],
  ])
  ...
  const createFieldHandlers = (field: Field) => {
    return {
      ...
      onBlur: (value: any) => {
        ...
        const timeInField = focusTime
          ? Math.round((Date.now() - focusTime) / 1000)
          : 0
    ...
    try {
      ...
      if (err.errors && Array.isArray(err.errors)) {
        const serverErrors = err.errors.map((e: any) => ({
          ...
          code: e.code,
          message:
            e.message?.[effectiveLocale] || e.message || 'Validation error',
        }))
    ...
    return (
      <div style={containerStyle}>
        ...
        >
          ...
          <p
            style={{ fontSize: '15px', color: COLORS.helper, lineHeight: 1.6 }}
          >
            {effectiveLocale === 'ar'
              ? 'يرجى الانتظار بينما نقوم بتحويلك إلى صفحة الدفع'
  ...
  if (submitted && form.thankYou?.show && !purchaseAuthConfig) {
    ...
    return (
      <div style={containerStyle}>
        ...
        >
          <h1 style={{ ...titleStyle, marginBottom: '1rem' }}>
            {form.thankYou.title?.[effectiveLocale]}
          </h1>
          <p
            style={{ fontSize: '15px', color: COLORS.helper, lineHeight: 1.6 }}
          >
            {form.thankYou.message?.[effectiveLocale]}
          </p>
        </div>
      </div>
    )
      ...
      <form onSubmit={handleSubmit} style={formStyle}>
        <div style={formContainerStyle}>
          {form.fields.map((f, index) => {
            ...
            const nextField =
              index < form.fields.length - 1 ? form.fields[index + 1] : null

            const isDateField = f.type === 'date'
            const isTimeField = f.type === 'time'
            const shouldGroupDateTime =
              isDateField && nextField?.type === 'time'
            ...
            if (shouldGroupDateTime) {
              ...
              const timeProps: any = {
                ...
                hasError:
                  errors.filter((er) => er.field === nextField.name).length > 0,
              }
              return (
                <div
                  key={f.attribute_key}
                ...
                >
                  ...
                  {fieldErrors.map((er) => (
                    ...
                    >
                      {er.message[effectiveLocale]}
                    </div>
                  ))}
                  {errors
                    .filter((er) => er.field === nextField.name)
                    .map((er) => (
                      <div
                        key={er.code}
                        style={{
            ...
            if (
              purchaseAuthConfig &&
              f.name === purchaseAuthConfig.adv_id_field &&
              userListings.length > 0
            ) {
              const fieldLabel =
                f.label?.[effectiveLocale] || f.label?.en || f.name
              ...
              return (
                ...
                >
                  <div style={galleryLabelStyle}>
                    ...
                    {isRequired && (
                      <span
                        style={{ color: COLORS.borderError, marginLeft: '4px' }}
                      >
                        *
                      </span>
                    )}
            ...
            const isItemIdField =
              purchaseAuthConfig && f.name === purchaseAuthConfig.item_id_field
                ...
                {isItemIdField && (
                  <div
                    style={{
                      ...
                      backgroundColor:
                        selectedItemPrice !== null ? '#EFF6FF' : '#F9FAFB',
                      borderRadius: '12px',
                      marginTop: '0.25rem',
                    }}
                  >
                    {loadingItemPrice ? (
                      ...
                      <span style={{ fontSize: '14px', color: COLORS.helper }}>
                        {effectiveLocale === 'ar'
                          ? 'اختر باقة لعرض السعر'
                          : 'Select a package to see price'}
                      </span>
                    )}
                  </div>
                )}
          ...
          >
            <div style={{ marginBottom: '0.75rem' }}>
              ...
              >
                {effectiveLocale === 'ar' ? (
                  <>
                    <li>
                      يجب أن يكون المحتوى المرسل متوافقًا مع سياسة النشر الخاصة
                      بتطبيق 4Sale.
                    </li>
                    <li>
                      تحتفظ 4Sale بالحق في رفض أي إعلان لا يتوافق مع الشروط
                      والسياسات.
                    </li>
                    <li>
                      في حالة رفض الإعلان، يتم استرجاع المبلغ إلى محفظة المستخدم
                      داخل التطبيق خلال 3 أيام عمل.
                    </li>
                    <li>
                      قيمة الخدمة تشمل تكلفة نشر المنشور بالإضافة إلى تكلفة
                      الترويج.
                    </li>
                    <li>
                      لا تتحمل 4Sale أي مسؤولية بخصوص بيع المركبة أو نتائج
                      الإعلان.
                    </li>
                    <li>
                      يتم إزالة المنشور أو القصة تلقائيًا بعد انتهاء المدة
                      المحددة في الباقة المختارة.
                    </li>
                    <li>الحد الأعلى للصور المسموح برفعها هو 5 صور فقط.</li>
                    <li>
                      4Sale هي المنوطة بتصميم الإعلان واختيار الصور التي سيتم
                      عرضها.
                    </li>
                    <li>
                  ...
                  <>
                    ...
                    <li>
                      4Sale reserves the right to reject any ad that doesn't
                      comply with terms and policies.
                    </li>
                    <li>
                      If an ad is rejected, the amount will be refunded to the
                      user's wallet within 3 business days.
                    </li>
                    <li>Service fee includes posting and promotion costs.</li>
                    <li>
                      4Sale is not responsible for vehicle sales or ad results.
                    </li>
                    <li>
                      Posts/stories are automatically removed after the selected
                      package duration ends.
                    </li>
            ...
            >
              <input
                ...
                onChange={(e) => {
                  console.log(
                    '[FormView] Terms checkbox changed:',
                    e.target.checked,
                  )
        ...
        >
          <button
            ...
            style={
              submitting || (purchaseAuthConfig && !termsAccepted)
                ? buttonDisabledStyle
                : buttonHover
                  ? buttonHoverStyle
                  : buttonStyle
